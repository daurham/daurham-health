...
describe('nutrition daily UX source', () => {
  it('keeps Add food primary, desktop split, and the existing bottom nav', () => {
    ...
    expect(page).toContain("params.get('action') !== 'add'")
    expect(page).toContain("next.set('date', resource.committedKey)")
    ...
    expect(page).toContain('NutritionPlaceholder')
    expect(page).toContain('page-enter')
