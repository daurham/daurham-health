  ...
  it('reaches every api handler and its server/domain dependencies', () => {
    ...
    expect(files).toEqual(
      expect.arrayContaining([
        ...
        'server/nutrition/gemini-jobs.ts',
        'server/integrations/gemini/client.ts',
        'server/integrations/gemini/config.ts',
        'server/nutrition/description-interpreter.ts',
        'src/domain/nutrition/interpret.ts',
    ...
    expect(files).not.toContain('server/backup/cli.ts')
    expect(files).toContain('server/handlers/backup-export.ts')
    expect(files).toContain('server/sleep/backfill.ts')
    expect(files).toContain('server/apple-health/hae-sleep-service.ts')
    expect(files).toContain('server/activity/queries.ts')
    expect(files).toContain('server/handlers/progress-activity.ts')
    expect(files).toContain('server/handlers/progress-sleep.ts')
