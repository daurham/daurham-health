...
describe('home-ai client', () => {
  ...
  it('POSTs nutrition label jobs to a distinct Home-AI path', async () => {
    const fetchImpl = vi.fn(async (url: string, init?: RequestInit) => {
      expect(url).toBe('https://ai.example.invalid/api/nutrition/label/jobs')
      expect(init?.method).toBe('POST')
      const headers = new Headers(init?.headers)
      expect(headers.get('x-api-key')).toBe('secret-home-ai-key')
      return new Response(JSON.stringify({ ok: true, job: { id: JOB_ID, status: 'queued' } }), {
    ...
    const created = await client(fetchImpl).createNutritionLabelJob({
      bytes: JPEG,
      filename: 'label.jpg',
      mimeType: 'image/jpeg',
    })
